...
public class enemy2 : MonoBehaviour
{
	...
	void Start()
	{
		platformWidth = this.GetComponent<BoxCollider2D>().size.x;
    ...
    void Update()
    {
		print ("enemy 2 script");
		_archibaldRB.velocity = new Vector2 (movespeed, 0.0f);
		if (transform.position.x == platformWidth) {
			_archibaldRB.velocity = new Vector2 (-movespeed, 0.0f);
		}
